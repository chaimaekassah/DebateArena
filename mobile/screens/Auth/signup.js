...
const SignUp = ({ navigation }) => {
  ...
  const handleSignUp = async (credentials) => {
    ...
    try {
      ...
      if (error.code === 'ECONNABORTED') {
        ...
        errorMessage =
          error.response.data || 'Email déjà utilisé ou données invalides';
  ...
  const testServerConnection = async () => {
    try {
      ...
      const testResponse = await axios.get('http://192.168.11.169:8080', {
        timeout: 5000,
      });
  ...
  const MyTextInput = React.forwardRef(
    ({ icon, isPassword, hidePassword, setHidePassword, ...props }, ref) => {
      return (
        <View>
          <Shadow>
            ...
            >
              {({ handleChange, handleBlur, handleSubmit, values }) => (
                <StyledFormArea>
                  ...
                  >
                    Une photo de profil par défaut vous sera automatiquement
                    attribuée.
                  </Label>
                </StyledFormArea>
              )}
